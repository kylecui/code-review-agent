...
This guide explains how to install, configure, and run Agent Review.
...
For standalone local scanning, you only need Python and an LLM API key.
...
Skip this section if you only run standalone local scans.
...
All configuration uses environment variables with the `AGENT_REVIEW_` prefix.
...
The agent uses [litellm](https://docs.litellm.ai/docs/providers) as the LLM abstraction layer. Set the API key variable that matches your provider:
...
For Azure, Vertex AI, Bedrock, and other providers, see the [litellm provider docs](https://docs.litellm.ai/docs/providers).
...
After you install the GitHub App and start the server, PR review runs automatically. The agent triggers on:

- `opened` - New PR created
- `synchronize` - New commits pushed to an existing PR
- `ready_for_review` - PR converted from draft to ready

The agent posts a **Check Run** (pass/fail in the PR checks tab). It also posts a **PR Review** with inline comments on relevant lines.

### Baseline Scan (GitHub)

You can scan an entire repository without waiting for a PR:
...
You can scan any local directory without GitHub credentials:
...
The agent checks for a repository-specific policy first. If none exists, it uses the default policy.
...
A finding is **blocking** when either condition is true:
...
The agent still runs collectors and posts findings as advisory. It does not block the merge.
...
The agent processes each scan in seven stages:
...
A deterministic file-pattern heuristic classifies changed files into categories (security, workflow, migration, api, docs, test, general). It also assigns:
...
Raw findings are converted to a canonical schema. Then they are deduplicated by fingerprint. If duplicates exist, the higher severity is kept.

### 5. LLM Reasoning

An LLM synthesizes findings. It prioritizes by impact, groups related issues, and generates human-readable explanations. It uses a three-tier strategy based on finding count (single call, chunked, or sampled). If the LLM fails or the cost budget is exceeded, the pipeline falls back to deterministic synthesis.
...
When a new commit is pushed to a PR with an in-progress review, the agent supersedes all active reviews for that PR. It then starts a new review on the new HEAD.

## Deployment

### Exposing the Webhook Endpoint

GitHub must reach your webhook URL over HTTPS. Available options:
...
LLM cost depends on provider and model. With `gpt-4o-mini` for classification and `gpt-4o` for synthesis, cost is typically about $0.01-$0.10 per review run, depending on diff size.

## Troubleshooting

**Webhook not received**
Check whether your webhook URL is publicly accessible. Then verify delivery status in GitHub App settings: Settings > Developer Settings > your app > Advanced > Recent Deliveries.

**401 Unauthorized on webhook**
The webhook secret in `.env` does not match the GitHub App configuration. Regenerate it and sync both sides.

**Pipeline stuck at COLLECTING**
A collector is timing out. Check `AGENT_REVIEW_SEMGREP_MODE` (set it to `disabled` if you do not have a Semgrep token). Verify network access to external APIs.

**LLM errors or empty synthesis**
Verify that the API key environment variable is set. If the budget is too low, increase `AGENT_REVIEW_LLM_COST_BUDGET_PER_RUN_CENTS`. Check logs for litellm error messages.

**No review posted on the PR**
Confirm that the GitHub App has Checks (Read & Write) and Pull requests (Read & Write) permissions. Verify that the app is installed on the target repository. Check that the PR is not a draft and was not opened by a bot.

**Template directory not found**
Set `AGENT_REVIEW_PROMPTS_DIR` to the absolute path of the `prompts/` directory. In Docker, the default `./prompts` is resolved relative to `WORKDIR /app`.
...
After deployment, open your instance URL. The login page appears. Register the first account. It is automatically promoted to admin (superuser).

### GitHub OAuth Configuration

To enable "Sign in with GitHub", complete these steps:
...
如果你只做本地独立扫描，只需要 Python 和 LLM API Key。
...
代理使用 [litellm](https://docs.litellm.ai/docs/providers) 作为 LLM 抽象层。你需要根据服务商设置对应的 API Key：
...
另一方面，Azure、Vertex AI、Bedrock 等其他服务商，请参考 [litellm 文档](https://docs.litellm.ai/docs/providers)。
...
安装 GitHub App 并启动服务后，PR 会自动进入审查。触发事件如下：

- `opened` - 创建新 PR
- `synchronize` - 向已有 PR 推送新提交
- `ready_for_review` - PR 从草稿转为就绪状态

代理会发布 **Check Run**（在 PR 的 checks 标签页显示通过/失败）。代理还会发布 **PR Review**（含行内评论）。
...
代理仍会运行收集器，并以建议形式发布发现。因此它不会阻止合并。
...
确定性的文件模式启发式方法会把变更文件分类为不同类别（security、workflow、migration、api、docs、test、general）。具体来说，还会分配：
...
系统会把各收集器的原始发现转换为统一格式。随后按指纹去重。若存在重复项，会保留更高严重性。

### 5. LLM 推理

LLM 会对发现做综合分析。它会按影响排序，关联相关问题，并生成可读解释。系统会根据发现数量使用三级策略（单次调用、分块处理或采样处理）。如果 LLM 失败，或者费用超出预算，则回退到确定性综合分析。
...
当新提交推送到一个仍在审查中的 PR 时，代理会取代该 PR 的所有活跃审查。然后基于新的 HEAD 重新开始。

## 部署

### 暴露 Webhook 端点

GitHub 需要通过 HTTPS 访问你的 Webhook URL。可用方案如下：
...
LLM 成本取决于服务商和模型。使用 `gpt-4o-mini` 做分类、`gpt-4o` 做综合分析时，每次审查通常约为 $0.01-$0.10。具体成本取决于 diff 大小。

## 故障排查

**收不到 Webhook**
检查 Webhook URL 是否可从公网访问。然后在 GitHub App 设置中查看投递状态：Settings > Developer Settings > 你的 App > Advanced > Recent Deliveries。

**Webhook 返回 401 Unauthorized**
`.env` 中的 Webhook secret 与 GitHub App 配置不一致。需要重新生成并同步。

**流水线卡在 COLLECTING 状态**
某个收集器超时。检查 `AGENT_REVIEW_SEMGREP_MODE`（没有 Semgrep Token 时应当设为 `disabled`）。确认网络可以访问外部 API。

**LLM 错误或综合分析为空**
确认 API Key 环境变量已设置。如果预算过低，可以增大 `AGENT_REVIEW_LLM_COST_BUDGET_PER_RUN_CENTS`。查看日志中的 litellm 错误信息。

**PR 上没有发布审查**
确认 GitHub App 具有 Checks（读写）和 Pull requests（读写）权限。确认 App 已安装到目标仓库。检查 PR 不是草稿，且不是 Bot 创建。

**找不到模板目录**
设置 `AGENT_REVIEW_PROMPTS_DIR` 为 `prompts/` 目录的绝对路径。在 Docker 中，默认 `./prompts` 相对于 `WORKDIR /app`。
...
部署完成后，访问实例 URL。登录页面会出现。注册第一个账号后，系统会自动将其升级为管理员（超级用户）。

### GitHub OAuth 配置

启用"使用 GitHub 登录"时，按以下步骤操作：
